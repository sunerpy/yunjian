...
- [词谱句式表：覆盖 2 支词牌，依据不是词谱](#词谱句式表覆盖-2-支词牌依据不是词谱)
...
## 词谱句式表：覆盖 2 支词牌，依据不是词谱

**这是本文档必须如实记录的一处未决问题，不要读成「词谱已就位」。**
...
1. **覆盖只有 2 支词牌**，而方案要求「v1 覆盖宋词三百首里出现的词牌」。表外词牌全部走标点回退。
2. **依据基础被换过。** 方案要求「公有领域词谱，含卷次页码」，实际用的是
   `source = corpus_modal`，即《全宋词》实测众数句式。**这是统计推断而非词谱权威**，且命中率
   不足一半（43.0% / 33.8%）说明「众数句式」本身对同名异体的代表性有限。
3. **根因是一个未决的来源问题。** 最完整的词谱数据 `Ci_Tunes.json` 抓自商业站点 `sou-yun.cn`，
   授权链未核实，已扣留（见[语料与索引](CORPUS.zh.md)）。扣留它的代价就是这张表。

`来源` 列的允许值是 `citune` 与 `corpus_modal` 两种，**v1 的 `citune` 行数为 0**。这一点由
诚实性测试钉住：`v1_claims_no_citune_authority`、`only_citune_claims_authority`、
`rhythm_source_identifiers_are_stable`。**标注为 `corpus_modal` 是诚实的，但它不满足方案写的
依据类型**——这一条留待补齐词谱覆盖并换回词谱依据时解决。
...
- **`coverage_advisory` 未开放。** 它要求一个独立的 KWS spike 先通过事先冻结的门槛
  ...
  被检测到」，不能证明用户说的是预期汉字。
- **词谱覆盖 2 支词牌，依据是实测众数而非公有领域词谱**（见上节）。
