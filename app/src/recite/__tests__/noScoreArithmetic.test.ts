...
describe("禁止前端算分", () => {
  ...
  it("不出现把比例换算成百分比的常见写法", () => {
    ...
    //
    // `100` 后面加一条否定预查：没有它的话 `* 1000`（秒换毫秒）会被当成百分比换算而误报。
    // todo 66 的语音面板要把 `<audio>` 的 `currentTime` 换成毫秒才能和拼接时间戳比对，
    // 那是**播放时钟的单位换算**，不是在比例上做算术——两者形似而语义无关，而这条门禁
    // 只该管后者。收窄之后它仍然会为一个真的 `ratio * 100` 变红。
    const hits = scan(/\*\s*100(?!\d)|100\s*\*|toFixed\(\s*[01]\s*\)\s*\+\s*["'`]%/);
