...
pub enum Error {
    ...
    /// 一条集评的出处缺了必填字段。
    ///
    /// **这个变体的存在本身就是需求。** 集评这条通道能存在的全部理由是「前现代评语
    /// 已过保护期，且逐条可复核」；一条不带出处的评语两个条件都不满足——它既无法
    /// 证明自己是前现代文本，也无法被核对。若把缺失的出处渲染成空字段，界面上就会
    /// 出现一段来路不明的文言，与被排除掉的第三方现代赏析在呈现上毫无区别。所以它
    /// 是独立的、可 `match` 的变体，并点名到底是哪一条集评的哪个字段，而不是
    /// [`Error::Corpus`] 里的一句字符串。
    #[error(
        "集评 {commentary_id}（诗 {poem_id}）的出处缺少 {missing_field}；集评必须与出处一同呈现，缺出处不得渲染"
    )]
    CommentaryCitationMissing {
        /// 出问题的那条集评的 id。
    ...
    /// 一条记录或一次配置未通过随包数据集的预生成门禁。
    ///
    /// **这个变体的存在本身就是需求。** 随包赏析是由我们分发的内容，它能被分发的
    /// 全部理由是「权重许可对产物再分发不设条件、每条都带溯源、且声明未经审校」。
    /// 把这些拒绝混进 [`Error::Corpus`] 的字符串里，调用方就无法区分「构建出错」
    /// 与「这条内容我们没有权利发」，而后者必须让整条流水线停下来。
    #[error("预生成门禁拒绝：{0}")]
    PregenerationRejected(String),

    /// 预生成被配置成使用闭源 API 供应商。
    ///
    /// **这个变体的存在本身就是需求。** 闭源 API 的输出附带我们未能核实的条款
    /// （OpenAI 的条款因 403 未核实，DeepSeek 的完全未核实），因此它不能用于生成
    /// 由我们再分发的数据集。这个判断与「许可字符串写错了」是两件不同的事，
    /// 消费方需要能分别 `match`，也需要错误文本直接点出开放权重要求。
    #[error(
        "预生成不得使用闭源 API 供应商 {provider}：随包数据集必须由可下载权重生成，\
         下载的权重不附带限制输出再分发的 API 条款"
    )]
    PregenerationClosedProvider {
        /// 被拒绝的闭源供应商标识。
        provider: String,
    },
